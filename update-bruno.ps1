...
elseif ( $RemoteLatestTag -gt $CurrentVersion -And $Now -gt $UpdateNotBefore ) {
    ...
    $TargetFileName = "bruno_$($RemoteLatestTag)_x64_win.zip"
    ...
    $CheckSumUrl="$($ToolRepoBaseUrl)/releases/expanded_assets/v$($RemoteLatestTag)"
    $CheckSum = "$(
                    ((Invoke-WebRequest -Uri "$($CheckSumUrl)").Content).Split("`n") `
                        | Select-String -Pattern "$($TargetFileName)" -SimpleMatch `
                        | Select-Object -Last 1
                )".Split("sha256:")[1].Split("`"")[0]
                # | Select-String -Pattern 'sha256:' -AllMatches `
