...
$ToolRepoBaseUrl = "https://github.com//eclipse-$($ToolName)/$($ToolName)-ide"
...
elseif ( $RemoteLatestTag -gt $CurrentVersion -And $Now -gt $UpdateNotBefore ) {
    ...
    $TargetFileName = "TheiaIDESetup-$($RemoteLatestTag).exe"
    $DownloadFullPath = "$($env:TEMP)\$($TargetFileName)"
    $OldPath = Get-Location

    $DownloadBaseUrl ="https://download.eclipse.org/theia/ide/$($RemoteLatestTag)/windows"
    $Url ="$($DownloadBaseUrl)/$($TargetFileName)"
    $CheckSumEncoded = "$([System.Text.Encoding]::UTF8.GetString(
                                                    (Invoke-WebRequest -Uri "$($DownloadBaseUrl)/latest.yml").Content
                                                    ).Split("`n") `
            | Select-String -Pattern 'sha512:' -AllMatches `
            | Select-Object -Last 1)".Split(" ")[1].Trim()
    $CheckSum = [System.BitConverter]::ToString([System.Convert]::FromBase64String($CheckSumEncoded)).Replace("-", "")
    ...
    $fileHash = Get-FileHash $DownloadFullPath -Algorithm SHA512

    if ($fileHash.Hash.ToLower() -eq $CheckSum.ToLower()) {
